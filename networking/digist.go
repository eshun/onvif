...
func NewTransport(username, password string, urlType string) *Transport {
	...
	temp := &http.Transport{}
	if urlType == "https" {
		temp.TLSClientConfig = &tls.Config{
...
func (t *Transport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	*req2 = *req
	...
	//fmt.Printf("[RoundTrip] ----第一次请求！---- %+v body:%v\n", *resp, string(body))
	//temp,_ := strconv.Atoi(resp.Header.Get("Content-Length"))
	//fmt.Printf("[RoundTrip] str = %v  int = %v\n", resp.Header.Get("Content-Length"), temp)
	////如果第一次请求包体有数据直接返回
	//if temp > 0 {
	//	return resp, err
	//}
